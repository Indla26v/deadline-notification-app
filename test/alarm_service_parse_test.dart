...
void main() {
  ...
  group('AlarmService.parseTimeFromText', () {
    ...
    Map<String, dynamic>? parse(String s) => service.parseTimeFromText(s);

    test('parses "Nov 1, 12:00 PM" (no at)', () {
      final result = parse('Meeting on Nov 1, 12:00 PM');
    ...
    test('parses "31 Oct 2025 4.00 pm" (dot time, explicit year)', () {
      final result = parse('Event: 31 Oct 2025 4.00 pm');
      expect(result, isNotNull);
      final dt = result!['date'] as DateTime;
      expect(dt.year, 2025);
      expect(dt.month, 10);
    ...
    test('parses "on 8th Nov by 8:30 am" (by preposition)', () {
      final result = parse('Reminder on 8th Nov by 8:30 am');
      expect(result, isNotNull);
      final dt = result!['date'] as DateTime;
      expect(dt.month, 11);
      expect(dt.day, 8);
    ...
    test('parses DATE/TIME block', () {
      final result = parse('Subject: Test\nDATE: 14TH NOV\nTIME: 6:00 PM');
      expect(result, isNotNull);
      final dt = result!['date'] as DateTime;
    ...
    test('parses "November 8 at 8:30 am"', () {
      final result = parse('Please join on November 8 at 8:30 am');
      expect(result, isNotNull);
      final dt = result!['date'] as DateTime;
      expect(dt.month, 11);
      expect(dt.day, 8);
